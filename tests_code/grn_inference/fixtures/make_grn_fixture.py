...
from hira.src.utils.util import basic_qc  # noqa: E402
...
def infer_for_cell_type(adata_full, cell_type, bulk_group):
    ...
    net = main_inference(X_norm, adata.var_names, WEIGHT_T)
    net['cell_type'] = cell_type
    net['sample_size'] = adata.shape[0]
    net['gene_size'] = adata.shape[1]
    net = net.sort_values(by='weight', ascending=False, key=abs).head(TOP_N_EDGES)
