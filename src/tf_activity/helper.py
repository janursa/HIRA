...
def determin_stats(adata, top_tfs):
    '''
        ...
        for dataset in datasets:
            ...
            if len(ages) > 1:
                ...
                if stats_df is not None:
                    ...
                    slope = stats_df_sub.loc[:, 'slope'].values[0]
                    r2 = stats_df_sub.loc[:, 'r2'].values[0]
                    intercept = stats_df_sub.loc[:, 'intercept'].values[0]
                    
                    p_value = stats_df_sub.loc[:, 'p_value'].values[0]
                    p_value_adj = stats_df_sub.loc[:, 'p_value_adj'].values[0]
                    p_value_adj = min([p_value_adj, 1])  # Ensure p-value is not greater than 1
                    spearman_corr = stats_df_sub.loc[:, 'spearman_corr'].values[0]
                else:
...
def stability_selection_shap(X, y,  top_k=10):
    """
    ...
    top_features_idx = np.argsort(feature_importances)[-top_k:]
...
def find_robust_predictors(adata, covariates, target, top_k=10):
    """
    ...
    # Stability selection
    top_features_idx = stability_selection_shap(X, y, top_k=top_k)
