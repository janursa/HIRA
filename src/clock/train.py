...
def build_model(reg_type, adata, tune_model, temp_dir):
    import anndata as ad
    ...
    if reg_type == 'tabpfn':
        ...
        seed_all(42)
        VAEAgeModel.setup_anndata(adata, batch_key='dataset', labels_key='age')
...
def wrapper_build_model_cell_type(cell_type, par):
    ...
    adata_all = merge_adata(datasets_training, feature_type, cell_type, data_type, age_limit=age_limit)
    ...
    model, _, _, y_trained   = build_model(reg_type, adata_all,  tune_model=par['tune_model'], temp_dir=par['temp_dir'])

    # Run CV again to get per-group scores
    if reg_type != 'NN':
        if False:
            X = adata_all.X
            y = adata_all.obs['age'].values
            if issparse(X):
                X = X.toarray()
            from scipy.stats import spearmanr
