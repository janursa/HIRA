...
def save_function(model, gene_names, cell_type, data_type, feature_type, reg_type, version):
    ...
    import joblib
    import numpy as np
    joblib.dump(model, os.path.join(save_dir, f"{cell_type}_{data_type}_{feature_type}_{reg_type}_model.pkl"))
    np.savetxt(f'{save_dir}/feature_names_{cell_type}_{data_type}_{feature_type}_{version}.txt', gene_names, fmt='%s')
...
def retrieve_function(reg_type, cell_type, data_type, feature_type, version):
    ...
    model = joblib.load(os.path.join(save_dir, f"{cell_type}_{data_type}_{feature_type}_{reg_type}_model.pkl"))
    gene_names = np.loadtxt(f'{save_dir}/feature_names_{cell_type}_{data_type}_{feature_type}_{version}.txt', dtype=str)
