...
def plot_overview_heatmap(stats, dataset, analysis_type, output_dir):
    ...
    if analysis_type == 'disease':
        # For disease, filter to both age groups
    ...
    _plot_single_heatmap(stats_filtered, palette, output_dir, suffix=dataset if analysis_type == 'disease' else None)
...
def plot_age_stratified_or_comparison(stats, stats_sig, dataset, analysis_type, target_cell_types, top_aging_tfs, output_dir):
    """
    Plot age-stratified analysis (disease) or aging-perturbation comparison.
    This function handles both disease and perturbation cases.
    """
    print(f"Generating {'age-stratified analysis' if analysis_type == 'disease' else 'aging vs perturbation comparison'}...")
...
def plot_case_studies(dataset, data_type, analysis_type, case_tfs, case_cell_type, target_cell_types, output_dir):
    """
    Plot case studies: healthy vs disease trends (disease) or donor-level effects (perturbation).
    """
    print(f"Generating {'case TF trends' if analysis_type == 'disease' else 'donor-level perturbation plots'}...")
...
def plot_pathway_analysis(stats, dataset, data_type, feature_type, analysis_type, pathway_cell_types, output_dir):
    ...
    if analysis_type == 'disease':
        # For disease, filter to younger age group
        stats_sig = stats_sig[
...
Examples:
  # Disease analysis
  python condition_analysis.py --dataset SLE_European --analysis-type disease --data-type bulk --feature-type tf_activity
  
  # Perturbation analysis
  python condition_analysis.py --dataset CXCL9 --analysis-type perturbation --data-type sc --feature-type tf_activity
        """
    )
    ...
    # 1. Overview heatmap
    plot_overview_heatmap(stats_sig if args.analysis_type == 'disease' else stats, 
                         args.dataset, args.analysis_type, output_dir)
    ...
    # 3. Aging-experiment heatmap (perturbation only)
    if args.analysis_type == 'perturbation' and not args.skip_heatmap and len(stats_sig) > 0:
